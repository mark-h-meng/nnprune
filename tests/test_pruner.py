...
class TestingPruning(unittest.TestCase):

    def test_chest_xray_model(self):
    ...
    def test_cifar_10_model(self):
        ...
        if k > 1:
            ...
            training_from_data.train_cifar_9_layer_cnn((train_features, train_labels),
                                                    ...
                                                    epochs=30,
                                                    topK=3)
            evaluator = Evaluator(k=3,epsilons = [0.05], batch_size=100)
        else:
            training_from_data.train_cifar_9_layer_cnn((train_features, train_labels),
                                                    (test_features, test_labels),
                                                    original_model_path,
                                                    ...
                                                    use_relu=True,
                                                    optimizer_config=optimizer)
            evaluator = Evaluator(epsilons = [0.05], batch_size=100)
    ...
    def test_cifar_10_model_top_k(self):
        k = 3
        original_model_path = 'paoding/models/cifar_10_cnn'
        pruned_model_path = 'paoding/models/cifar_10_cnn_pruned'
